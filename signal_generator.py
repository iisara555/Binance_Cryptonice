...
class SignalGenerator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.strategies: Dict[str, StrategyBase] = {
            ...
            "scalping": ScalpingStrategy(self.config.get("scalping", {})),
        ...
        self.risk_config = {
            ...
            "max_risk_score": rc.max_risk_score,
            "min_strategies_agree": rc.min_strategies_agree,
            ...
            "independent_strategy_execution": rc.independent_strategy_execution,
        }
        ...
        self._signal_cache_ttl: float = rc.signal_cache_ttl
        self._signal_cache_max_size: int = rc.signal_cache_max_size
        ...
        self._strategy_perf_lookback_days: int = rc.strategy_perf_lookback_days
        self._mtf_cache: Dict[str, tuple] = {}
        self._mtf_cache_lock = threading.Lock()
        self._mtf_cache_ttl: float = rc.mtf_cache_ttl_seconds
        self._mtf_cache_max_size: int = rc.mtf_cache_max_size
        self._mtf_indicator_cache: Dict[tuple, tuple] = {}
        self._mtf_indicator_cache_lock = threading.Lock()
        self._mtf_indicator_cache_ttl: float = rc.mtf_indicator_cache_ttl_seconds
        self._mtf_indicator_cache_max_size: int = rc.mtf_indicator_cache_max_size
    ...
    def refresh_risk_config_for_mode(self, mode: str) -> None:
        ...
        if "independent_strategy_execution" in profile:
            ...
            self.risk_config["independent_strategy_execution"] = bool(strategies_cfg["independent_strategy_execution"])
    ...
    def generate_signals(
        self, data: pd.DataFrame, symbol: str, use_strategies: Optional[List[str]] = None
        ...
        with self._signal_cache_lock:
            if len(self._signal_cache) >= self._signal_cache_max_size:
                ...
                sorted_keys = sorted(self._signal_cache.keys(), key=lambda k: self._signal_cache[k][1])
        ...
        all_signals, strategy_reject_reasons = collect_raw_trading_signals(
            ...
            strategy_names=strategies_to_use,
            data=data,
            symbol=symbol,
            diag_fn=_diag,
            emit_sniper_diagnostics=self._emit_sniper_diagnostics,
        )
        ...
        if not aggregated:
            ...
            _diag(
                ...
                + (f", strategy_rejects={strategy_reject_reasons}" if strategy_reject_reasons else ""),
            )
        else:
    ...
    def generate_sniper_signal(
        ...
        agg.trade_rationale = str(
            (raw_signal.metadata or {}).get("trade_rationale") or self._generate_trade_rationale(agg)
        )
        return [agg]
    ...
    def _make_aggregate_for_direction(
        ...
        if weight_sum <= 0:
            return None
        weighted_conf = (
            sum(s.confidence * strategy_weights.get(s.strategy_name, 1.0) for s in type_signals) / weight_sum
        )
    ...
    def _aggregate_signals(
        self, signals: List[TradingSignal], market_condition: MarketCondition, symbol: str, data: pd.DataFrame
    ) -> List[AggregatedSignal]:
        ...
        if self.risk_config.get("independent_strategy_execution"):
            ...
            for raw in signals:
                if raw.signal_type == SignalType.HOLD:
                    continue
                built = self._make_aggregate_for_direction([raw], raw.signal_type, market_condition, symbol, data)
        ...
        for sig_type, type_signals in by_type.items():
            built = self._make_aggregate_for_direction(type_signals, sig_type, market_condition, symbol, data)
            if built is not None:
        ...
        for sig in signals:
            ...
            if strat_stats and strat_stats.get("total", 0) >= 5:
                ...
                _diag(
                    ...
                    "StratWeight:Dynamic",
                    "INFO",
                    f"{sig.strategy_name} win_rate={win_rate:.2f} " f"(n={strat_stats['total']}) -> w={w:.2f}",
                )
    ...
    def _adjust_for_market_condition(self, confidence: float, signals: List[TradingSignal], condition: Any) -> float:
    ...
    def _calculate_risk_score(self, signals: List[TradingSignal], market_condition: Any) -> float:
    ...
    def check_risk(self, signal: AggregatedSignal, portfolio: Dict[str, Any]) -> SignalRiskCheck:
        """
        ...
        if signal.combined_confidence < self.risk_config["min_confidence"]:
            result.passed = False
            reason = (
                f"Confidence {signal.combined_confidence:.2f} below minimum " f"{self.risk_config['min_confidence']}"
            )
        ...
        if signal.risk_score > self.risk_config["max_risk_score"]:
            result.passed = False
            reason = f"Risk score {signal.risk_score:.0f} exceeds maximum " f"{self.risk_config['max_risk_score']}"
            result.reasons.append(reason)
            ...
            _diag(pair, "RiskCheck:RiskScore", "PASS", f"risk_score={signal.risk_score:.0f} (warning: elevated)")
        else:
            _diag(
                pair,
                "RiskCheck:RiskScore",
        ...
        if self.risk_config.get("independent_strategy_execution"):
            ...
            result.passed = False
            reason = (
                f"Only {len(signal.strategy_votes)} strategies agree, need "
            ...
            _diag(
                ...
                "RiskCheck:StrategyAgreement",
                "PASS",
                f"{len(signal.strategy_votes)} strategies agree " f"(min {self.risk_config['min_strategies_agree']})",
            )
        ...
        if signal.signal_type == SignalType.SELL:
            ...
            result.passed = False
            reason = f"Risk-reward ratio {signal.avg_risk_reward:.2f} below minimum " f"{MIN_RISK_REWARD_RATIO}"
            result.reasons.append(reason)
            _diag(pair, "RiskCheck:RiskReward", "REJECT", reason)
        elif signal.avg_risk_reward < 1.5:
            result.warnings.append(f"Suboptimal risk-reward ratio: {signal.avg_risk_reward:.2f}")
            _diag(
                pair,
                "RiskCheck:RiskReward",
                "PASS",
            ...
            _diag(pair, "RiskCheck:RiskReward", "PASS", f"R:R={signal.avg_risk_reward:.2f} >= {MIN_RISK_REWARD_RATIO}")
        ...
        # ── Final verdict ──
        if result.passed:
            _diag(pair, "RiskCheck:Final", "PASS", "All 8 risk gates passed — signal approved for trading")
        else:
            _diag(pair, "RiskCheck:Final", "REJECT", f"Blocked by: {'; '.join(result.reasons)}")
    ...
    def get_best_signal(
        self, data: pd.DataFrame, symbol: str, portfolio: Dict[str, Any], use_strategies: Optional[List[str]] = None
    ) -> Optional[AggregatedSignal]:
        """
        ...
        if not signals:
            ...
            _diag(symbol, "GetBestSignal", "REJECT", "No signals generated at all")
            return None
        ...
        _diag(symbol, "GetBestSignal", "REJECT", f"All {len(signals)} signal(s) failed risk checks")
        return None
    ...
    def generate_mtf_signals(
        self, pair: str, timeframes: Optional[List[str]] = None, db=None
        ...
        if timeframes is None:
            timeframes = self.config.get("multi_timeframe", {}).get("timeframes", ["1m", "5m", "15m", "1h"])
        ...
        # Generate result
        result = MultiTimeframeResult(pair=pair, timestamp=datetime.now(timezone.utc), timeframes=mtf_data)
    ...
    def get_mtf_signal(
        ...
        metadata = {
            ...
            "higher_timeframe_trend": (
                mtf_result.higher_timeframe_trend.value if mtf_result.higher_timeframe_trend else None
            ),
