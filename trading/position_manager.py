...
class PositionManager:
    ...
    def __init__(self, api_client: BinanceThClient, trade_executor: TradeExecutor, config: Dict[str, Any]):
    ...
    def _get_exchange_open_order_rows(self, symbols: List[str]) -> tuple[List[Dict[str, Any]], set[str]]:
        ...
        for symbol in symbols:
            ...
            for row in list(open_orders or []):
                ...
                normalized["entry_price"] = _safe_float(
                    row.get("rat") or row.get("rate") or row.get("entry_price"), 0.0
                )
    ...
    def add_position(self, position: Position) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Added position {position.order_id} | {position.symbol} {position.side} @ {position.entry_price:.2f}"
            )
    ...
    def get_open_positions(self, symbol: Optional[str] = None) -> List[Position]:
        ...
        with self._lock:
            positions = [pos for pos in self._positions.values() if pos.is_open]
