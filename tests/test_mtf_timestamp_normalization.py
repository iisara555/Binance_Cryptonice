...
def test_generate_mtf_signals_reuses_closed_candle_indicators_while_forming_candle_moves():
    ...
    with patch('multi_timeframe.TechnicalIndicators.calculate_rsi', side_effect=lambda close: pd.Series([65.0] * len(close))) as mock_rsi, \
         patch('multi_timeframe.TechnicalIndicators.calculate_macd', side_effect=lambda close: (
             pd.Series([1.0] * len(close)),
             pd.Series([0.5] * len(close)),
             pd.Series([0.5] * len(close)),
         )) as mock_macd, \
         patch('multi_timeframe.TechnicalIndicators.calculate_adx', side_effect=lambda high, low, close: pd.Series([25.0] * len(close))) as mock_adx, \
         patch('multi_timeframe.TechnicalIndicators.calculate_atr', side_effect=lambda high, low, close: pd.Series([1.0] * len(close))) as mock_atr:
        first = generator.generate_mtf_signals(
            pair='THB_DOGE',
            timeframes=['1m'],
            db=db,
