...
class TestRiskManagement:
    ...
    def test_atr_increases_with_volatility(self):
        ...
        atr_low = calculate_atr(low_vol["high"].tolist(), low_vol["low"].tolist(), low_vol["close"].tolist(), period=14)

        atr_high = calculate_atr(
            high_vol["high"].tolist(), high_vol["low"].tolist(), high_vol["close"].tolist(), period=14
        )
    ...
    def test_get_default_sl_tp(self):
        ...
        assert sl_pct_alt == -4.0
        assert tp_pct_alt == 7.0
    ...
    def test_sl_tp_with_atr(self):
        ...
        sl, tp = rm.calc_sl_tp_from_atr(entry_price=entry, atr_value=atr, direction="long", risk_reward_ratio=2.0)
...
class TestSignalGenerator:
    ...
    def test_aggregate_signals(self, sample_price_data):
        ...
        signals = generator.generate_signals(
            data=sample_price_data, symbol="THB_BTC", use_strategies=["trend_following", "momentum"]
        )
    ...
    def test_sniper_ignores_forming_candle_macd_cross(self):
        ...
        with patch("signal_generator.TechnicalIndicators.calculate_macd", side_effect=macd_side_effect), patch(
            "signal_generator.TechnicalIndicators.calculate_atr", return_value=pd.Series(np.full(len(data), 5.0))
        ):
...
class TestIntegration:
    ...
    def test_full_signal_to_execution_flow(self, sample_price_data, mock_api_client):
        ...
        from risk_management import RiskConfig, RiskManager
        from signal_generator import SignalGenerator
        from trade_executor import ExecutionPlan, OrderSide, TradeExecutor

        # Step 1: Generate signals
        generator = SignalGenerator()
        signals = generator.generate_signals(data=sample_price_data, symbol="THB_BTC", use_strategies=["momentum"])
        ...
        if signals:
            signal = signals[0]
            portfolio = {"balance": 50000, "positions": []}
...
class TestEdgeCases:
    ...
    def test_zero_division_handling(self):
        ...
        atr = calculate_atr(data["high"].tolist(), data["low"].tolist(), data["close"].tolist(), period=14)
