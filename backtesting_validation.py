...
class BacktestingValidator:
    ...
    def validate_strategy(
        self, strategy, historical_data: pd.DataFrame, initial_capital: float = 10000.0, run_walk_forward: bool = True
    ) -> ValidationReport:
        """
    ...
    def _generate_feedback(self, result: BacktestResult, checks: Dict[str, bool]) -> Tuple[List[str], List[str]]:
        ...
        if not checks["sufficient_trades"]:
            warnings.append(
                f"Insufficient trade samples: {result.total_trades} trades (minimum {self.MINIMUM_REQUIRED_TRADES} required)"
            )
        ...
        if not checks["acceptable_drawdown"]:
            warnings.append(
                f"Excessive drawdown: {result.max_drawdown:.1%} (maximum {self.MAX_ALLOWED_DRAWDOWN:.0%} allowed)"
            )
            recommendations.append("Implement stricter stop-loss rules or reduce position sizing")

        if not checks["acceptable_sharpe"]:
            warnings.append(
                f"Low Sharpe ratio: {result.sharpe_ratio:.2f} (minimum {self.MINIMUM_SHARPE_RATIO} required)"
            )
